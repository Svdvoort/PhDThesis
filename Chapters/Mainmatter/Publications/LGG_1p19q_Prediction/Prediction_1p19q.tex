...
\chapter[Predicting the \acl{1p19qstat} of presumed low-grade glioma with an externally validated machine learning algorithm][AI 1p/19q prediction]{Predicting the \acl{1p19qstat} of presumed low-grade glioma with an externally validated machine learning algorithm}\label{chap:LGG1p19q}


\begin{ChapterAbstract}
    \textbf{Purpose:} Patients with \acl{1p19qcodel} \cgls{LGG} have longer overall survival and better treatment response than patients with \acl{1p19qint} tumors.
    Therefore, it is relevant to know the \acl{1p19qstat}.
    To investigate whether the \acl{1p19qstat} can be assessed prior to tumor resection, we developed a machine learning algorithm to predict the \acl{1p19qstat} of presumed \cgls{LGG} based on preoperative \cgls{MRI}.
...
\cGls{LGG} is a primary brain tumor that originates from glial cells.
...
\begin{enumerate}
\item Diffuse \cgls{IDH} wild-type astrocytoma (\cgls{IDH} wild-type, \acl{1p19qint})
\item Diffuse \cgls{IDH} mutant astrocytoma (\cgls{IDH} mutated, \acl{1p19qint})
...
In addition, studies have suggested that residual tumor has a more negative impact on survival in \acl{1p19qint}, \cgls{IDH}-mutated astrocytomas than on \acl{1p19qcodel}, \cgls{IDH}-mutated oligodendrogliomas \autocite{wijnenga2017impact, clark2019extent}.
...
Features such as frontal tumor localization, indistinct tumor borders, heterogeneous \cgls{SI} on \cgls{T2} images, and both cortical and subcortical tumor infiltration all suggest the presence of \acl{1p19qcotion} \autocite{smits2017imaging}.
...
This is not an ideal test set as the \acl{1p19qstat} is not clinically relevant for high-grade glioma, and there is a selection bias of \cgls{IDH}-mutated tumors only.
...
All patients aged 18 years or older newly diagnosed with presumed \cgls{LGG} and who underwent tumor resection or biopsy between October 2002 and March 2017 at the \cgls{EMC} or the \cgls{HMC} were retrospectively included in the \cgls{EMC}/\cgls{HMC} dataset.
...
All tumors were subclassified on the basis of the \cgls{WHO} 2016 criteria.
...
Presumed \cgls{LGG} was defined as nonenhancing tumor, as seen on the presurgical \cgls{T1C} \cgls{MR} image.
...
Tumor segmentation was performed by two independent observers (F. Incekara and G. Kapas) using ITK-Snap \autocite{yushkevich2006user}.
...
Molecular analysis of the \acl{1p19qstat} was performed with \cgls{FISH} for all tumors; \cgls{IDH} mutation status was not determined.
...
All tumors were semiautomatically segmented by M. Smits on the \cgls{T2} images using ITK-Snap.
...
To predict the \acl{1p19qstat} of the tumors based on \cgls{MRI} features, the PREDICT toolbox was used.
This toolbox was used to extract a total of 78 image features (such as image intensity, tumor texture, tumor shape, and tumor location) from the \cgls{T1C} and \cgls{T2} \cgls{MR} image.
...
% TODO replace Figures in this tikz figure by their original code
\begin{figure}
    \centering
    \resizebox{\textwidth}{!}{
...
To compare the results of the algorithm with expert performance, the \acl{1p19qstat} of the \cgls{TCIA} tumors was also predicted by two neuroradiologists and two neurosurgeons at the Erasmus MC Brain Tumor Centre.
...
Median tumor volume in the \cgls{EMC}/\cgls{HMC} dataset was significantly larger than in the \cgls{TCIA} dataset (median: \SI{47.80}{\centi\meter\cubed}, \cgls{IQR}: \num{58.65} vs.\ median \SI{35.70}{\centi\meter\cubed}, \cgls{IQR}: \num{49.10}, \pvalue{= 0.04}).
There were fewer \acl{1p19qcodel} tumors in the \cgls{EMC}/\cgls{HMC} compared with the \cgls{TCIA} dataset (\per{35.20} vs.\ \per{65.40}, \pvalue{< 0.0001}).
...
\begin{tabular}{L{4cm} S[table-number-alignment=center-decimal-marker, table-format=3.0] S[table-number-alignment=center-decimal-marker, table-format=3.1] S[table-number-alignment=center-decimal-marker, table-format=3.0] S[table-number-alignment=center-decimal-marker, table-format=3.1] S[table-comparator = true]}
    ...
    \bfseries{Clinical}\\
    \hspace{1em}Age median [IQR] (years) & \multicolumn{2}{c}{\IQR{43}{17}} & \multicolumn{2}{c}{\IQR{39}{19.5}} & 0.11\\
    \hspace{1em}Sex& & & & &0.45\\
    \hspace{2em}Male & 161 & 56.7 & 68 & 52.7\\
    \hspace{2em}Female & 123 & 43.3 & 61 & 47.3\\

    \bfseries{Imaging}\\
    \hspace{1em}Volume median [IQR] (\si{\centi\meter\cubed})& \multicolumn{2}{c}{\IQR{47.8}{58.7}} & \multicolumn{2}{c}{\IQR{35.7}{49.1}} & 0.04\\
    ...
    \bottomrule
\end{tabular}
\caption{Patient and tumor characteristics. Abbreviations: \acrcaption{IQR}, \acrcaption{NGS}, \acrcaption{FISH}, \acrcaption{MLPA}}\label{tab:LGG_1p19q_characteristics}
\end{table}
...
\begin{tabular}{L{2cm} C{3.3cm} C{3.3cm} S[table-comparator = true]}
    \toprule
    & \thead{\acrshort{EMC}/\acrshort{HMC}\\Training set\\(\numbersamples{=284})} & \thead{\acrshort{TCIA}\\Validation set\\(\numbersamples{=129})} & {\thead{P-value}} \\
    \midrule
    Accuracy & \meanconf{0.698}{0.636}{0.760} & \meanconf{0.693}{0.657}{0.729} & 0.872\\
    AUC & \meanconf{0.755}{0.694}{0.817} &\meanconf{0.723}{0.708}{0.737} & 0.313\\
    F1-score & \meanconf{0.701}{0.640}{0.761} & \meanconf{0.697}{0.661}{0.733} & 0.896\\
    Precision & \meanconf{0.570}{0.491}{0.649} & \meanconf{0.787}{0.754}{0.820} & <0.001\\
    Sensitivity & \meanconf{0.657}{0.562}{0.752} & \meanconf{0.732}{0.689}{0.775} & 0.123\\
    Specificity & \meanconf{0.721}{0.628}{0.813} & \meanconf{0.717}{0.544}{0.691} & 0.027\\
    \bottomrule
\end{tabular}
...
According to the algorithm, the most important features for accurate \acl{1p19qstat} prediction were the cranial/caudal location of the tumor, the skewness of the \cgls{T2} \cgls{SI} histogram, and one of the texture features, together with age and sex \cref{fig:LGG_1p19q_feature_importance}.
The algorithm identified a typical \acl{1p19qcodel} glioma as a frontal heterogeneous tumor as seen on \cgls{T1C} and \cgls{T2} scans, whereas a typical \acl{1p19qint} glioma was identified as a parietal homogeneous tumor, as shown in \cref{fig:LGG_1p19q_Examples}.
...
Indeed, as our results suggest, it is even for experienced neuro-oncologic surgeons and radiologists a challenge to accurately predict the \acl{1p19qstat} of nonenhancing tumors based on preoperative \cgls{MR} images (\cgls{AUC} of \numrange{0.45}{0.83}).
...
Second, it is suggested that postsurgical residual \acl{1p19qint}, \cgls{IDH}-mutated tumor has a more negative impact on survival than residual \acl{1p19qcodel}, \cgls{IDH}-mutated (oligodendroglioma) tumor \autocite{wijnenga2017impact, clark2019extent}.
With presurgical knowledge of the specific molecular subtype, the surgeon can make a better informed decision on whether or not to push the limits of resection at the time of surgery, avoiding on the one hand reresection in case of residual \acl{1p19qint}, \cgls{IDH}-mutated tumor and less-justified postsurgical deficits in \acl{1p19qcodel} tumor on the other hand.
...
Therefore, for our future work, we are expanding our database with more patients in whom the tumor \cgls{IDH} status is known to eventually be able to predict all clinically relevant subtypes of presumed \cgls{LGG}.
...
This pipeline takes the \cgls{T1C} \cgls{T2} \cgls{MR} images, tumor segmentations, and brain segmentations on both scans, and labels indicating \acl{1p19qstat} for each patient which can then be used to train a \cgls{SVM} \autocite{cortes1995support}.
...
Rotation invariant \cglspl{LBP} were used to describe the texture of the tumor \autocite{ojala2002multiresolution}.
...
The \cgls{LBP} was determined for each slice of the \cgls{MRI} scan in which tumor was present.
The results over all pixels within the tumor and slices containing tumor were then concatenated and consequently the mean, standard deviation, median, kurtosis and skewness, and peak value were calculated for each setting, resulting in 18 \cgls{LBP} features.
...
Tumor location was determined by using the \cgls{COM} from the brain mask and \cgls{COM} of the tumor segmentation.
The anterior/posterior, left/right and inferior/superior coordinate of the vector pointing from the \cgls{COM} of the brain to the \cgls{COM} of the tumor were then included as features, resulting in 3 location features.
...
\begin{table}
    ...
    \begin{tabular}{L{3.9cm}R{0.3cm}L{5.7cm}}
        ...
        \multicolumn{3}{l}{\textit{Derived from tumor segmentation and brain mask}}\\
        ...
        \multicolumn{3}{l}{\textit{Derived from tumor segmentation}}  \\
...
\begin{tabular}{L{2cm} C{4cm} C{4cm}}
    ...
    & Mean (\per{95} \acrshort{CI}) & Mean (\per{95} \acrshort{CI})\\
    \midrule
    \textbf{Accuracy} &  \meanconf{0.717}{0.670}{0.764} & \meanconf{0.698}{0.636}{0.706}\\
    \textbf{\acrshort{AUC}} & \meanconf{0.780}{0.729}{0.830} & \meanconf{0.755}{0.694}{0.817}\\
    \textbf{F1-score} & \meanconf{0.717}{0.670}{0.763} & \meanconf{0.701}{0.640}{0.761}\\
    \textbf{Precision} & \meanconf{0.676}{0.620}{0.732} & \meanconf{0.570}{0.491}{0.649}\\
    \textbf{Sensitivity} & \meanconf{0.710}{0.637}{0.783} & \meanconf{0.657}{0.562}{0.752}\\
    \textbf{Specificity} & \meanconf{0.723}{0.658}{0.788} & \meanconf{0.721}{0.628}{0.813}\\
    \bottomrule
\end{tabular}
