...
\\
\textbf{\acrshort{WHO} 2016 categorization}\\
...
It can be seen that for the \gls{LGG} the network focused on a bright rim in the \gls{FLAIR} scan, whereas for the \gls{HGG} it focused on the enhancement in the \gls{T1C} scan.
...
We have developed a method that can predict the \gls{IDH} mutation status, 1p/19q co-deletion status, and grade of glioma, while simultaneously providing the \gls{tumor} segmentation, based on preoperative \gls{MRI} scans.
For the genetic and histological feature predictions, we achieved an \gls{AUC} of 0.90 for the \gls{IDH} mutation status prediction, an \gls{AUC} of 0.85 for the 1p/19q co-deletion prediction, and an \gls{AUC} of 0.81 for the grade prediction, in the test set.
...
In conclusion, we have developed a non-invasive method that can predict the \gls{IDH} mutation status, 1p/19q co-deletion status, and grade of glioma, while at the same time segmenting the \gls{tumor}, based on preoperative \gls{MRI} scans with high overall performance.
...
\end{table}
 ...
 \end{table}
  ...
  \multirow{5}{*}{\rotatebox[origin=c]{90}{\textbf{Actual}\hspace{65pt}}}
      ...
      Only one patient (TCGA-HT-A5RC) was predicted as a non-existing category. It was predicted as an \acrshort{IDH}-wildtype, 1p/19q co-deleted, grade IV \gls{tumor}.
